...
typedef enum {
    ...
    PPC_OP_PS_CMPO1,
    PPC_OP_PS_SEL,
    /* All FPU-touching operations (they raise the FP-unavailable exception
     * when MSR[FP] is clear) are kept contiguous from PPC_OP_STFIWX through
     * PPC_OP_PS_SEL; add new FP/paired-single ops inside that block. */
    PPC_OP_MULLW,
    PPC_OP_MULLWO,
    PPC_OP_MULHW,
    PPC_OP_MULHWU,
...
// true for FPU-touching operations, which must raise the FP-unavailable
// exception instead of executing while MSR[FP] is clear (Gekko lazy FP
// context switching depends on this)
