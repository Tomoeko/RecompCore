...
static void emit_fcompare(FILE* out, const PPCInst* inst) {
    /* ppc_fcmp mirrors Dolphin's Helper_FloatCompare{Ordered,Unordered}:
     * FPCC + CR field + VXSNAN/VXVC exception semantics. */
...
/* FP loads/stores go through the ppc_lfs/lfd/stfs/stfd helpers, which
 * mirror Dolphin's word-alignment exception and the lfs/stfs
 * ConvertToDouble / ConvertToSingle bit conversions. A false return means
 * an exception was taken: the block stops and the update-form RA
...
/* ---- Guest-cycle accounting (generated-code ABI v2) -------------------- */

/* Keep in sync with the PPC_OP_MFSPR emission switch below: SPRs listed here
 * are modeled in CPUState; everything else routes to the instruction
 * fallback (and is therefore charged by the environment, not the block). */
...
/* Keep in sync with the PPC_OP_MTSPR emission switch below. */
...
/* True when the emitted code for this instruction is an unconditional
 * `ppc_fallback_instruction(...); return;` — the environment executes it
 * (and accounts its cycles), so its block cost is zero and it always ends
...
/* True when the emitted code unconditionally leaves the linear instruction
 * stream (control transfer or environment fallback): the next instruction
 * begins a new accounting block. Conditional early exits (traps, exception
 * checks) do NOT end a block — their fall-through path continues and the
 * block was already charged in full, exactly like Dolphin's block-granular
 * JIT downcount. */
...
/* Per-instruction guest cycle cost, mirroring Dolphin's PPCTables
 * GekkoOPTemplate num_cycles exactly (dolphin/Source/Core/Core/PowerPC/
 * PPCTables.cpp) so chassis timing matches Dolphin's interpreter/JIT
 * charging. Instructions that route to the environment fallback cost zero
 * here: Dolphin's interpreter step (or the chassis cache-op fast path)
 * charges them on delivery. */
...
static void emit_instruction_with_range(FILE* out, const PPCInst* inst,
    ...
    /* Gekko lazy FP context switching: FPU instructions must raise the
     * FP-unavailable exception instead of executing while MSR[FP] is clear,
     * so the OS can restore the owning thread's FP context and retry. */
    ...
    case PPC_OP_PS_NEG:
        /* Sign flips operate on the raw f64 lane bits (Dolphin ps_neg):
         * no rounding through f32 and no NaN quieting. */
        fprintf(out, "    ctx->fpr[%u] = dolrecomp_f64_from_bits(dolrecomp_f64_to_bits(ctx->fpr[%u]) ^ 0x8000000000000000ull);\n",
                inst->rD, inst->rB);
    ...
    /* ps_merge: raw f64 lane moves, no rounding, no FPSCR (Dolphin). The
     * temporaries keep rD==rA/rB aliasing correct. */
    ...
    case PPC_OP_PS_SEL:
        /* Compares the full doubles (Dolphin ps_sel); raw lane copies. */
        fprintf(out, "    {\n");
    ...
    case PPC_OP_STSWI:
    case PPC_OP_STSWX: {
        /* ppc_stsw mirrors Dolphin's Helper_StoreString (aligned word RMW). */
    ...
    case PPC_OP_STWCX:
        /* Dolphin stwcxd: alignment exception, exact reserve_address match,
         * reservation cleared only on success, CR0 = EQ|SO / SO. */
        fprintf(out, "    {\n        u32 ea = ");
        emit_xform_ea(out, inst->rA, inst->rB, false);
...
void emit_function(FILE* out, const PPCInst* insts, u32 count, u32 func_addr) {
    ...
    /* Guest-cycle accounting: partition the function into accounting blocks
     * (leaders = entry, local branch targets, and successors of control
     * transfers) and charge each block's full Dolphin-mirrored cost once at
     * its top via `ctx->downcount -= n`. Intra-function flow only enters
     * blocks at leaders (local gotos target branch targets; dispatcher
     * re-entry PCs are branch targets, post-call addresses, or exception
     * retry points), so every executed block is charged exactly once; the
     * only imprecision is a mid-block exception retry, which re-charges the
     * faulting block — matching real-hardware retry cost. */
