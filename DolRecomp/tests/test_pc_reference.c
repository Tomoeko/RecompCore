...
static void test_psq_memory(CPUState* cpu) {
    ...
    /* Dolphin quantizes NaN to 0 (SType(clamp(NaN)) == 0 on arm64), not to
     * the max like a saturating clamp would. */
    check_eq(mem_read8(cpu, base + 0xD0), 0x00, "psq_st NaN quantizes to zero");
    ...
    /* Dolphin's psq_l/psq_st never raise alignment exceptions (Helper_De/
     * Quantize read/write unaligned via the MMU); type-0 unaligned is fine. */
    check_eq(cpu->exception & PPC_EXC_ALIGNMENT, 0, "psq_l f32 unaligned allowed");
